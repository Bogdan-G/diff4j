...
public final class ContextualPatch {
    ...
    private void backup(File target) throws IOException {
        if (target.exists()) {
            copyStreamsCloseAll(new BufferedOutputStream(new FileOutputStream(computeBackup(target))), new BufferedInputStream(new FileInputStream(target)));
        }
    }
    ...
    private void writeFile(SinglePatch patch, List<String> lines) throws IOException {
        ...
        if (patch.binary) {
            if (patch.hunks.length == 0) {
                ...
                copyStreamsCloseAll(new BufferedOutputStream(new FileOutputStream(patch.targetFile)), new ByteArrayInputStream(content));
            }
        } else {
            PrintWriter w = new PrintWriter(new OutputStreamWriter(new BufferedOutputStream(new FileOutputStream(patch.targetFile)), getEncoding(patch.targetFile))); 
            try {
